...
def fit_model_gp_RBF(dataset):
    dataset_x = dataset['x']
    dataset_y = dataset['y']

    rbf_kernel = ScaleKernel(RBFKernel(lengthscale=1.0))

    model = SingleTaskGP(dataset_x, dataset_y, covar_module=rbf_kernel)
...
def init_dataset(initNum, objDim, bounds, objFunc):
    ...
    preference, regret = get_preference_infor(a, aa, objFunc)
    x = torch.cat([a, aa], 1)
